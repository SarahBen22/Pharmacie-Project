...
class CommandesModel extends Model {
    ...
    private $num_commande;
	  ...
	  private $total;
		...
		public function createOne ($id_client, $num_commande, $date_de_commande,$total){

		$db=parent::connect();

		 // on recherche si ce login est déjà utilisé par un autre membre
		 $sql = 'SELECT * FROM commandes WHERE id_client="$id_client)"';
		 ...
		 if (empty($data)) {// si rien ds le 1
				$sql = 'INSERT INTO commandes VALUES(0, "$id_client","$num_commande","$date_de_commande","$total")';
      ...
      public function setTotal( $total ){
		...
		//UPDATE
			public function update(CommandesModel $comm){
				...
				$sql= "UPDATE comm SET num_commande = :num_commande, date_de_commande = :date_de_commande, id_client = :id_client, total = :total WHERE id=".$comm->id();
				$query= $db -> prepare ($sql);
        ...
        $query->bindValue(':total', $comm->total());
